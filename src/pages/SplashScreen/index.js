...
class SplashScreen extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      prevProps.updateMessage !== this.props.updateMessage &&
      this.props.updateMessage == 'Update installed. Restarting app..'
    ) {
      setTimeout(() => {
        codePush.restartApp();
      }, 1000);
    ...
    if (
      prevProps.codePushErr !== this.props.codePushErr &&
      this.props.codePushErr
    ) {
      this.setState({
        isError: true,
      });
  ...
  componentDidMount() {
    ...
    if (Platform.OS === 'android') {
      ...
      this.linking = Linking.addEventListener('url', this.handleDeepLink);
    }
  }
  handleDeepLink = e => {
    if (e) {
