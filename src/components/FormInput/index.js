...
class FormInput extends Component {
  ...
  getPlaceholder() {
    language.transformText(this.props.placeholder).then((res) => {
      this.setState({placeholder: res});
    });
  }

  _renderLabel(props) {
    if (this.state.isFocused == false && props.value?.length == 0) {
      return null;
    } else {
      return <StaticText style={styles.label} property={props.label} />;
    }
  }
        ...
        if (this.state.isSecret == true) {
          return (
            <TouchableOpacity
              ...
              onPress={this.showPasswordHandler}>
              <Image
                resizeMode={'contain'}
      ...
      return (
        <>
          <TextInput
            ...
            numberOfLines={
              this.state.multiline ? this.props.numberOfLines : null
            }
          ...
          />
          {this.props.multiline ? null : (
            <View style={styles.underline(this.props.type)} />
          )}
