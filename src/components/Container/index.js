    ...
    if (Platform.OS === 'ios') {
      return (
        <View style={styles.container(this.props.containerColor)}>
          ...
          <SafeAreaView
            style={styles.container(this.props.containerColor)}
            forceInset={{top: 40, bottom: -10}}>
            <KeyboardAvoidingView
              behavior="padding"
        ...
        <View style={styles.container(this.props.containerColor)}>
          ...
          <SafeAreaView style={styles.container(this.props.containerColor)}>
            <View style={styles.contentContainer(this.props.noBackground)}>
              {this.props.children}
              <Loading modalVisible={this.props.network.isLoading} />
              <ServerError
                modalVisible={this.props.network.isServerError}
                closeModal={this.closeModalServerError}
              />
...
const mapDispatchToProps = (dispatch) => ({
  set_error_status: (p) =>
    dispatch(actions.network.reducer.set_error_status(p)),
  set_success_status: (p) =>
    dispatch(actions.network.reducer.set_success_status(p)),
  set_network_error_status: (p) =>
    dispatch(actions.network.reducer.set_network_error_status(p)),
  set_server_error_status: (p) =>
    dispatch(actions.network.reducer.set_server_error_status(p)),
});
