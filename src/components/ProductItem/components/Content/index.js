...
class Content extends PureComponent {
  ...
  renderQuotaClaim() {
    ...
    if (Number(this.props.data.quota_claim) > 0) {
      claimLimit =
        Number(this.props.data.quota_claim) -
        Number(this.props.data.total_claim_product || 0);
    }
    ...
    if (
      ...
      return (
        <Text style={styles.text.claimLimit}>Limit Promo: {claimLimit}</Text>
      );
    }
  }
        ...
        <View style={{paddingHorizontal: this.props.dashboard ? 15 : 0}}>
          ...
          (this.props.data.on_promo == 1 &&
            ...
            <Text style={styles.text.price.promo(this.props.dashboard)}>
              <StaticText property={'productList.content.price'} />
              {productPrice}
            </Text>
          ) : (
