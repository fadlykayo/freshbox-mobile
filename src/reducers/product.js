...
const reorderTransaction = (state, payload) => {
  ...
  for (let i = 0; i < payload.data.length; i++) {
    ...
    if (cartIndex == -1) {
      ...
      if (
        payload.data[i].product.total_claim_product === undefined ||
        payload.data[i].product.quota_claim === 0
      ) {
        ...
        if (
          parseInt(payload.data[i].product.total_claim_product) ===
          payload.data[i].product.quota_claim
        ) {
          payload.data[i].product.count = payload.data[i].qty;
      ...
      if (
        newCart[cartIndex].count.total_claim_product === undefined ||
        newCart[cartIndex].count.quota_claim === 0
      ) {
